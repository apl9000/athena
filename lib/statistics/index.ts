...
import { INVALID_ARGUMENT_ERROR, SAME_LENGTH_ERROR } from '../errors/index';
import { sumOfSquares, dotProduct } from '../linear_algebra/index';
...
export const mode = (xs: number[]): number[] => {
	...
	Object.keys(counted).forEach(
		(value) => counted[value] === max && mode.push(Number(value))
	);
	return mode;
};
